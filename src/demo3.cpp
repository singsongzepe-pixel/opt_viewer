...
private:
    ...
    static void mouseCallback(GLFWwindow *window, double xposIn, double yposIn) {
        auto app = reinterpret_cast<SpinningCube*>(glfwGetWindowUserPointer(window));
        if (app) {
...
#ifdef NDEBUG
        cout << "createDescriptorSetLayout finished" << endl;
#endif
    }
    ...
    void createGraphicsPipeline() {
        ...
        if (descriptorSetLayout == VK_NULL_HANDLE) {
            cout << "descriptorSetLayout is nullptr" << endl;
        }
#endif
...
#ifdef NDEBUG
        cout << "createGraphicsPipeline finished" << endl;
#endif
    }
    ...
    void loadTorus() {
        ...
        for (int i = 0; i <= mainSegments; ++i) {
            ...
            for (int j = 0; j <= tubeSegments; ++j) {
                ...
                float cx = mainRadius * cos(u);
                float cy = mainRadius * sin(u);
                float cz = 0.0f; 
                Vertex vertex{};
                vertex.pos = {x, y, z};
                // color gradient
                vertex.color = {0.5f + 0.5f * cos(u), 0.5f + 0.5f * sin(v), 0.5f};
                vertex.normal = glm::normalize(glm::vec3(x-cx, y-cy,z-cz));
...
#ifdef NDEBUG
        cout << "createIndexBuffer called" << endl;
#endif
...
#ifdef NDEBUG
        cout << "cleanupSwapChain called" << endl;
#endif
